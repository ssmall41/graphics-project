...
automaticMipmapsEnabled(enableAutomaticMipmaps?1:0)
{
	...
	if (numColorBufs > 0)
	{
		...
		for (int i=0; i<numColorBufs; i++)
		{
			...
			BindBuffer();
			glFramebufferTexture2DEXT( GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT+i, GL_TEXTURE_2D, colorIDs[i], 0 );
	...
	if (hasZbuf > 0)
	{
		...
		if (type == GL_TEXTURE_CUBE_MAP)
		{
			...
			glTexImage3D(type, 0, GL_DEPTH_COMPONENT, width, height, depth, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
		else if (type == GL_TEXTURE_2D || type == GL_TEXTURE_1D_ARRAY_EXT)
			glTexImage2D(type, 0, GL_DEPTH_COMPONENT, width, height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
		else if (type == GL_TEXTURE_1D)
			glTexImage1D(type, 0, GL_DEPTH_COMPONENT, width, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
		BindBuffer();
...
void FrameBuffer::DisplayAsFullScreenTexture( int attachment, float lod )
{
