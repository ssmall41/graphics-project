...
public:
	SceneDefinedUI();           // Set up a default UI mapping (pretty dumb, though...)
	...
	inline int Key  ( int uiFunction ) const { return uiFunction > 0 && uiFunction < __UI_FUNCTIONS ? keyMap[uiFunction] : 0; }
	inline int Mouse( int uiFunction ) const { return uiFunction > 0 && uiFunction < __UI_FUNCTIONS ? keyMap[uiFunction] : 0; }
